...
function getFunctionBody(func) {
  const fullFuncString = func !== undefined ? func.toString() : ''; // 🔥 Преобразуем в строку
  return fullFuncString;
}
/**
...
function getArgumentsCount(funcs) {
  return funcs.map((arr) => arr.length);
}
...
function getPolynom(...array) {
  return (x) => {
    return array.reduce(
      (sum, coef, index) => sum + coef * x ** (array.length - index - 1),
      0
    );
  };
}
...
function partialUsingArguments(fn, ...initialArgs) {
  return (...laterArgs) => fn(...initialArgs, ...laterArgs);
}
